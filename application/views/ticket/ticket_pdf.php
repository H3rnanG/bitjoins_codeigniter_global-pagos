...
if(strlen($formato)>0){
    ...
    if($print){
        ...
        if($edt_sbt){
            $edt_sbt_1 = number_format($edt_sbt, 0, '.', ',');
            $pt2_pos2 = 50.7;
    ...
    // 2DA PANTALLA
    if($mnt_total){
        $mnt_total = number_format($mnt_total, 2, '.', ',');
        $pt2_pos1 = 170;
        $obpdf->Image($imgpath, 175, $pt2_pos1, 30, 3, 'JPG', '', '', false, 300, '', false, false, 0, false, false, false);
    ...
    if($edt_sbt){
        $edt_sbt = number_format($edt_sbt, 2, '.', ',');
        $pt2_pos2 = 178.2;
        $obpdf->Image($imgpath, 175, $pt2_pos2, 30, 3, 'JPG', '', '', false, 300, '', false, false, 0, false, false, false);
    ...
    if($edt_appago){
        ...
        $obpdf->SetFont('Helvetica', '', '7');
        $obpdf->Image($imgpath, 175, 190, 30, 2.8, 'JPG', '', '', false, 300, '', false, false, 0, false, false, false);
        $obpdf->writeHTMLCell(35.2, 3, 172, 190, $edt_tc, 0, 0, 1, true, 'R', false); 

        $obpdf->Image($imgpath, 175, 196, 30, 4.2, 'JPG', '', '', false, 300, '', false, false, 0, false, false, false);
        $obpdf->writeHTMLCell(35.2, 3, 172, 196, $edt_appago, 0, 0, 1, true, 'R', false); 
    }
