 ...
 */
class Agencia_peso_model extends CI_Model {
	var $id = 'id';
	var $tabla = 'calculadora_agencia_peso';
    var $arrWhere = array();

    var $column_order = array(null,null,'ca.nombre','ap.tipo','ap.peso', 'ap.zona1', 'ap.zona2', 'ap.zona3', 'ap.zona4', 'ap.zona5', 'ap.zona6');
    var $column_search = array('ca.nombre','ap.peso');
    var $order = array('calculadora_agencia_id, tipo, peso'=>'ASC');
    ...
    function get_datatables_vista()
    {
        ...
        $this->db->select('ap.id,ap.tipo, ap.peso, ap.zona1, ap.zona2, ap.zona3, ap.zona4, ap.zona5, ap.zona6, ap.calculadora_agencia_id, ca.nombre as agencia');
        $this->db->from($this->tabla.' as ap');
        $this->db->join('calculadora_agencia as ca','ca.id = calculadora_agencia_id', 'left');
